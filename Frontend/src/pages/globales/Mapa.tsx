...
// Componente que restringe el registro a dentro del polígono
  ...
  useMapEvents({
    click(e) {
      ...
      if (poly.getBounds().contains(point)) {
        ...
        alert("No puedes registrar un punto fuera del área delimitada.");
      }
    },
  });
  return null;
...
const MapaPage: React.FC = () => {
  ...
  const handleDelete = (punto: PuntoMapa) => {
    console.log("Abriendo modal de eliminación para punto:", punto);
    setSelectedPunto(punto);
  ...
  return (
    <DefaultLayout>
      <div className="container mx-auto p-6 flex flex-col gap-6">
                ...
                >
                  <Popup>
                    <div>
                      <b>{punto.nombre}</b>
                      <br />
                      <span>{punto.descripcion || "Sin descripción"}</span>
                      <br />
                      Lat: {`${(lat || 0).toFixed(6)}`} // Línea 219
                      <br />
                      Lng: {`${(lng || 0).toFixed(6)}`} // Línea 220
                    </div>
                  </Popup>
                </Marker>
              );
        ...
        >
          <p>¿Estás seguro de eliminar este punto? Esta acción es irreversible.</p>
        </ReuModal>
      </div>
    </DefaultLayout>
  );
