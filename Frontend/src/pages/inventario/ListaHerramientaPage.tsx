  ...
  return (
    <DefaultLayout>
      ...
      >
        {selectedHerramienta && (
          <>
            <ReuInput label="Nombre" placeholder="Ingrese el nombre" type="text" value={selectedHerramienta.nombre} 
              onChange={(e) => setSelectedHerramienta({ ...selectedHerramienta, nombre: e.target.value })} />
            <ReuInput label="Descripción" placeholder="Ingrese la descripción" type="text" value={selectedHerramienta.descripcion} 
              onChange={(e) => setSelectedHerramienta({ ...selectedHerramienta, descripcion: e.target.value })} />
            <ReuInput label="Cantidad" placeholder="Ingrese la cantidad" type="number" value={selectedHerramienta.cantidad.toString()} 
              onChange={(e) => setSelectedHerramienta({ ...selectedHerramienta, cantidad: Number(e.target.value) })} />
            <ReuInput label="Estado" placeholder="Ingrese el estado" type="text" value={selectedHerramienta.estado} 
              onChange={(e) => setSelectedHerramienta({ ...selectedHerramienta, estado: e.target.value })} />
            <div className="mb-4 flex items-center">
              <input type="checkbox" checked={selectedHerramienta.activo} 
      ...
      <ReuModal isOpen={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen} 
        title="¿Estás seguro de eliminar esta herramienta?" onConfirm={handleConfirmDelete}>
