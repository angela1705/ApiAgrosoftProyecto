...
class InsumoConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        print(f"WebSocket desconectado para {self.group_name}, código: {close_code}")
    ...
    def check_insumos(self):
        ...
        for insumo in insumos:
            ...
            if insumo.cantidad <= umbral_cantidad:
                insumo_hash = hashlib.md5(f"{insumo.id}low_stock{timestamp}".encode()).hexdigest()
            ...
            if insumo.fecha_caducidad and (insumo.fecha_caducidad - today).days <= umbral_dias:
                insumo_hash = hashlib.md5(f"{insumo.id}expiring{timestamp}".encode()).hexdigest()
                notifications.append({
                    'id': insumo_hash,
                    'type': 'expiring',
    ...
    async def send_initial_notifications(self):
        try:
            print(f"Checking insumos for {self.group_name} on connect")
